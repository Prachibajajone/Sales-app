...
const Register=async(req,res)=>{
  ...
  const userfind = await SalesUser.findOne({ $or: [{ email: email }, { password: password }] });
  try {
    if (userfind) {
        return res.json({ message: 'User Already Exists' })
    } else {
        const userWithHashedPassword = { firstName, lastName, email, password: hashedPassword }
      const newUser = await new SalesUser({ ...userWithHashedPassword }).save();
      console.log(newUser);
...
const Login=async(req,res)=>{
    ...
    const userfind = await SalesUser.findOne({ email: email });
    if (userfind) {
